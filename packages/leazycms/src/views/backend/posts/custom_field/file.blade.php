...
@if($post->user_id==Auth::user()->id)<a  title="Hapus dokumen untuk mengganti" data-toggle="tooltip"  class="fa fa-trash text-danger" style="cursor:pointer" onclick="media_destroy('{{ $field[_us($r[0])]}}')"></a><br> @endif
@else
<input {{ isset($r[2]) ? 'required':'' }} accept="{{ $post->type=='surat-keluar' && _us($r[0]) == 'file_surat' ? 'application/pdf' : allow_mime() }}" type="file" class="form-control-sm form-control" value="{{ $field[_us($r[0])]??null }}" name="{{_us($r[0])}}" placeholder="Entri {{$r[0]}}">
@endif
@endif
